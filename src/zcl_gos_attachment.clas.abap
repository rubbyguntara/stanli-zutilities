*--------------------------------------------------------------------*
* Created on         : 30.11.2023 14:35:05
* Created by         : A. Lutfiadi
* Functional         : -
* Description        : GOS Attachment
*--------------------------------------------------------------------*
CLASS zcl_gos_attachment DEFINITION
  ...
  PUBLIC SECTION.
    TYPES:
      tt_notes       TYPE STANDARD TABLE OF zpmst1004,
      tt_attachments TYPE STANDARD TABLE OF zpmst1005.
    ...
    METHODS get_notes
      ...
      EXPORTING
        !et_notes       TYPE zcl_gos_attachment=>tt_notes .
    METHODS get_attachments
      EXPORTING
        !et_attachments TYPE zcl_gos_attachment=>tt_attachments .
      ...
      CASE l_obj_type.
        WHEN cl_fitv_gos=>gc_type_file.
          ...
          CASE l_file_ext.
            ...
            WHEN 'DOC' OR 'DOCX'.
              ls_atta-icon_name = 'ICON_DOC'.
            WHEN 'RTF'.
              ls_atta-icon_name = 'ICON_RTF'.
            WHEN 'MSG'.
              ls_atta-icon_name = 'ICON_MSG'.
            WHEN 'DOT'.
              ls_atta-icon_name = '~Icon/DocumentFileTemplate'.
            WHEN 'GIF' OR  'PNG'.
              ls_atta-icon_name = '~Icon/ImageFile'.
            WHEN 'JPG' OR 'JPE' OR 'JPEG'.
              ls_atta-icon_name = 'ICON_JPG'.
            WHEN 'VSD'.
              ls_atta-icon_name = 'ICON_VSD'.
            WHEN 'TIF'.
              ls_atta-icon_name = 'ICON_TIF'.
            WHEN 'BMP'.
              ls_atta-icon_name = 'ICON_BMP'.
            WHEN 'FAX'.
              ls_atta-icon_name = 'ICON_FAX_DOC'.
            WHEN 'XLS' OR 'XLSX' OR 'XLV'.
              ls_atta-icon_name = '~Icon/SpreadsheetFile'.
            WHEN 'HTM' OR 'HTML'.
              ls_atta-icon_name = '~Icon/HtmlFile'.
            WHEN 'PPT' OR 'PPTX'.
              ls_atta-icon_name = '~Icon/PresentationFile'.
            WHEN 'TXT' OR 'WRI' OR 'LWP'.
              ls_atta-icon_name = '~Icon/PlaintextFile'.  "GLW note 2628387
            WHEN 'PDF'.
              ls_atta-icon_name = '~Icon/PdfFile'.
            WHEN 'HLP'.
              ls_atta-icon_name = '~Icon/HelpFile'.
            WHEN 'HTT'.
              ls_atta-icon_name = '~Icon/HtmlFileTemplate'.
            WHEN OTHERS.
...
* Begin of AGU Note [1699983] - WEB: UWL Notepad attachment issue / txt file not read
...
*    l_doc_info TYPE sofolenti1,
      ...
      l_data_read          TYPE i,
* End of AGU Note [1699983] - WEB: UWL Notepad attachment issue / txt file not read
...
*         E_PSERVER   =
      ...
      et_messages = cl_fitv_gos=>is_extension_allowed( i_doctype = '!SAPDUMMY!' i_extension = file_ext file_size = size ).  "GLW note 2440451
...
*   prepare header
      ...
      CLEAR ls_header.
*    ls_header = '&SO_FORMAT=BIN'.      "AGU Note [1699983] - WEB: UWL Notepad attachment issue / txt file not read
*    APPEND ls_header TO lt_obj_header. "AGU Note [1699983] - WEB: UWL Notepad attachment issue / txt file not read
...
* it's a note
        ...
        l_retype = 'NOTE'.
        l_obj_type = 'RAW'.
*      l_object_hd_change-file_ext = 'TXT'. "AGU Note [1699983] - WEB: UWL Notepad attachment issue / txt file not read
...
*  l_obj_rolea-logsys = is_lporb-catid. "CAL2122355

*  l_object_id_fol  = l_folder_id.
*  l_object_id = ls_object_id.  "AGU Note [1699983] - WEB: UWL Notepad attachment issue / txt file not read
*  CONCATENATE l_object_id_fol l_object_id INTO l_obj_roleb-objkey RESPECTING BLANKS. "AGU Note [1699983] - WEB: UWL Notepad attachment issue / txt file not read
    " l_obj_roleb-objkey = l_doc_info-doc_id(34). "AGU Note [1699983] - WEB: UWL Notepad attachment issue / txt file not read
    ...
    CALL FUNCTION 'BINARY_RELATION_CREATE'
      ...
      EXCEPTIONS
        OTHERS       = 1.
    IF sy-subrc = 0.
* commit  commented for OCA[2398290]
      IF iv_commit_on = 'X'. "OCA[]
        lv_pernr = is_lporb-instid+0(8).
        lv_reinr = is_lporb-instid+8(10).
        DATA(lv_perio) = CONV ptrv_perod( is_lporb-instid+18(3) ).
        PERFORM update_ptrv_shdr IN PROGRAM sapmp56t USING  "GLW note 2540018
                                  lv_pernr lv_reinr lv_perio 1.
        COMMIT WORK AND WAIT.
      ENDIF.
*    wd_this->MV_UPDATE_FLAG = 'X'.
      gv_attachments_changed = abap_true. "GLW note 1976133
    ELSE.
      MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
              WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4 INTO ls_message-message.
      ls_message-type = sy-msgty.
